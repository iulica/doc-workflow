...
class MailMergeTask(BasePlugin):
    ...
    def _init_document(self, value_dict):
        ...
        self._document = MailMerge(
            input_docx_filename, auto_update_fields_on_open="auto"
        )
    ...
    def do_task(self, value_dict, **kwargs):
        ...
        data_dict = {
            field_name: self._workbook.get_value(
                self._sheet, row, self._column_map[field_name.lower()]
            )
    ...
    def finish(self):
        # print(merge_data)
        separator = self._task_info.get("separator", "nextPage_section")
